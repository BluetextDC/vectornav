...
using BT_Widgets.Mvc.Models.PackagingOptionsBlock;

namespace BT_Widgets.Mvc.Controllers
{
    //[EnhanceViewEngines]
    [ControllerToolboxItem(Name = "PackagingOptionsBlock", Title = "Packaging-Options-Block", SectionName = "Test_BT")]
    public class PackagingOptionsBlockController : Controller
    {
        ...
        {
            int cnt = 0;
            List<PackagingOptionsBlock> m_ListObject = new List<PackagingOptionsBlock>();
            this.ViewBag.ListTitle = this.ListTitle;
            this.ViewBag.ListType = this.ListType;

            foreach (var strval in this.DeserializeItems())
            {
                PackagingOptionsBlock m_object = new PackagingOptionsBlock();
