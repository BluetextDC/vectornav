...
using BT_Widgets.Mvc.Models.FullwidthFiftyFifty;

namespace BT_Widgets.Mvc.Controllers
{
    //[EnhanceViewEngines]
    [ControllerToolboxItem(Name = "FullwidthFiftyFifty", Title = "Full-width 50-50", SectionName = "Blue Text")]
    public class FullwidthFiftyFiftyController : Controller
    {
        ...
        {
            int cnt = 0;
            List<FullwidthFiftyFifty> m_ListObject = new List<FullwidthFiftyFifty>();
            this.ViewBag.ListTitle = this.ListTitle;
            this.ViewBag.ListType = this.ListType;

            foreach (var strval in this.DeserializeItems())
            {
                FullwidthFiftyFifty m_object = new FullwidthFiftyFifty();
